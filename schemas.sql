...
--     CONSTRAINT unique_daily_activity UNIQUE (user_id, activity_id, completion_date)
-- );

-- -- Create function to set completion_date
...
--     BEFORE INSERT OR UPDATE ON public.activity_completions
--     FOR EACH ROW
--     EXECUTE FUNCTION set_completion_date();

-- -- Create indexes for better query performance
-- CREATE INDEX IF NOT EXISTS idx_activity_completions_user_date 
--     ON public.activity_completions (user_id, completion_date);
-- CREATE INDEX IF NOT EXISTS idx_activity_completions_activity 
--     ON public.activity_completions (activity_id);

-- -- Insert default activities
-- INSERT INTO public.activities (name, description, points) VALUES
--     ('daily_checkin', 'Complete your daily mood check-in', 10),
--     ('mood_journal', 'Write in your mood journal', 15),
--     ('track_mood', 'Complete the mental health questionnaire', 20);
...
create table public.activity_completions (
  ...
  constraint unique_daily_activity unique (user_id, activity_id, completion_date),
  constraint activity_completions_activity_id_fkey foreign KEY (activity_id) references activities (id),
  constraint activity_completions_user_id_fkey foreign KEY (user_id) references users (user_id)
) TABLESPACE pg_default;

create index IF not exists idx_activity_completions_user_date on public.activity_completions using btree (user_id, completion_date) TABLESPACE pg_default;

create index IF not exists idx_activity_completions_activity on public.activity_completions using btree (activity_id) TABLESPACE pg_default;

create trigger set_completion_date_trigger BEFORE INSERT
or
update on activity_completions for EACH row
execute FUNCTION set_completion_date ();
...
create table public.mood_entries (
  ...
  user_id uuid null,
  mood_type character varying(20) not null,
  emoji_code character varying(10) null,
  reasons character varying(50) [] null,
  notes text null,
  entry_timestamp timestamp without time zone null default CURRENT_TIMESTAMP,
  entry_date date GENERATED ALWAYS as ((entry_timestamp)::date) STORED null,
...
create index IF not exists idx_questionnaire_answers_response on public.questionnaire_answers using btree (response_id) TABLESPACE pg_default;
...
create table public.questionnaire_summaries (
  ...
  insights text not null,
  recommendations text not null,
  breathing_exercise_id integer null,
  created_at timestamp without time zone null default CURRENT_TIMESTAMP,
  constraint questionnaire_summaries_pkey primary key (summary_id),
  constraint questionnaire_summaries_breathing_exercise_id_fkey foreign KEY (breathing_exercise_id) references breathing_exercises (id),
  constraint questionnaire_summaries_response_id_fkey foreign KEY (response_id) references questionnaire_responses (response_id),
  constraint questionnaire_summaries_severity_level_check check (
    (
      (severity_level)::text = any (
        array[
          ('mild'::character varying)::text,
          ('moderate'::character varying)::text,
          ('severe'::character varying)::text,
