...
void main() {
	group('UPD-PD-01: A logged-in user\'s dashboard displays role-specific metrics and welcome message', () {
		test('Student dashboard loads with personalized greeting and student-specific widgets', () async {
			...
			// Load dashboard data
			final dashboardData = await mockLoadDashboardData(
				userId: user.id,
		...
		test('Dashboard data is role-specific and does not contain cross-role widgets', () async {
			...
			final studentDashboard = await mockLoadDashboardData(
				userId: studentUser.id,
				userType: studentUser.userType,
			);
			
			final counselorDashboard = await mockLoadDashboardData(
				userId: counselorUser.id,
				userType: counselorUser.userType,
			);
		...
		test('Invalid user type throws exception', () async {
			...
			expect(
				() => mockLoadDashboardData(userId: user.id, userType: user.userType),
				throwsException,
			);
		});
